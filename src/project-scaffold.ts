...
const REPO_ROOT = process.env.WAELIO_BUILDER_ROOT ?? path.resolve(__dirname, '..', '..');
...
export function scaffoldProject(projectsDir: string, projectName: string): string {
...
export function scaffoldFromBlueprint(projectsDir: string, projectNames: string[]): string[] {
  return projectNames.map((projectName) => scaffoldProject(projectsDir, projectName));
}
